/* weaves 
   ...
   I've modified this to switch on regex library type.
   Currently only works for POSIX regex.
...
*/
{
  ...
  printf("ok(%d).\n", nsub);
  for (i = 0; i < nsub; i++) {
    printf("%02d: reg(%d, %d, \"%.*s\").\n", i, pmat[i].rm_so, pmat[i].rm_eo,
                  pmat[i].rm_eo - pmat[i].rm_so, tomatch + pmat[i].rm_so);
    if (pmat[i].rm_so < 0) break;
  }
}
